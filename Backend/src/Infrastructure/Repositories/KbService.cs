...
public class KbService : IKbService
{
    ...
    public async Task<KbFixBug> AnalyzeCodeSyntax(string request)
    {
    ...
    public async Task<KbSolver> AnalyzeCodeSolver(string request)
    {
        // Đường dẫn file input/output nằm ngay trong folder Python
        string inputPath = Path.Combine(_pythonDir, _task2InputFile);
        string outputPath = Path.Combine(_pythonDir, "solver_result.json"); // Tên file này do Python hardcode
