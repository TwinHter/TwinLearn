...
public class ProblemController(ProblemService problemService) : ControllerBase
{
    ...
    {
        var problemDto = await problemService.GetProblemByIdAsync(id);
        if (problemDto == null)
    ...
    {
        var problems = await problemService.GetFilteredProblemsAsync(filter);
    ...
    {
        var newProblemDto = await problemService.CreateProblemAsync(createDto);
    ...
    {
        var updatedProblemDto = await problemService.UpdateProblemAsync(id, updateDto);
    ...
    {
        var result = await problemService.DeleteProblemAsync(id);
        if (!result)
            return NotFound("Không tìm thấy problem.");
        return Ok();
    }
